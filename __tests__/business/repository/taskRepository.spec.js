...
describe("TaskRepository", () => {
  ...
  describe("- ERROR CASES -", () => {
    ...
    it("completeTask | Ensure object validation failed", async () => {
      ...
      const result = await taskRepository.completeTask(objectTask);
      expect(result).toBe(new BadRequest("Complete Task Repository | the task object is not in a correct format").message);
    ...
    it("completeTask | Ensure the description length is greater than 2500", async () => {
      ...
      const result = await taskRepository.completeTask(objectTask);
      expect(result).toBe(new BadRequest("Complete Task Repository | the description size is too big").message);
      expect(new BadRequest().status).toBe(400);
    ...
    it("completeTask | Ensure object validation failed", async () => {
      ...
      const result = await taskRepository.completeTask(objectTask);
      expect(result).toBe(new NotFoundError("Complete Task Repository | no tasks found for this id").message);
      expect(new NotFoundError().status).toBe(404);
